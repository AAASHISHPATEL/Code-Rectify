...
const App = () => {
  ...
  const ai = new GoogleGenAI({ apiKey: import.meta.env.VITE_GOOGLE_GEMINI_KEY }); // replace "YOUR_API_KEY" with you api key
  const fixCodeAi = new GoogleGenAI({ apiKey: import.meta.env.VITE_GOOGLE_GEMINI_KEY }); // replace "YOUR_API_KEY" with you api key
...
`,
  ...
  async function FixCode() {
    ...
    const response = await fixCodeAi.models.generateContent({
      model: "gemini-2.0-flash",
      contents: `You are an expert-level software developer, skilled in writing efficient, clean, and advanced code.
I’m sharing a piece of code written in ${selectedOption.value}.
Your job is to deeply review this code and provide the following:
...
`,
    ...
    const lang = selectedOption.value; // or any other language name dynamically
    const regex = new RegExp("```" + lang + "|```", "g");
    // console.log(response.text.replace(regex, ''));
    setCode(response.text.replace(regex, ''));
      ...
      <div className="main flex justify-between" style={{ height: "calc(100vh - 90px" }}>
        <div className="left h-[87.5%] w-[50%]">
          <div className="tabs !mt-5 !px-5 !mb-3 w-full flex items-center gap-[10px]">
            ...
            className="btnNormal bg-zinc-900 min-w-[120px] transition-all hover:bg-zinc-800">{!fixedCodeLoading ? "Fix Code" : "..."}</button>
            <button onClick={() => {
              if (code === "") {
                alert("Please enter code first")
              }
